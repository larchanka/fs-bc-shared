...
function fetchRecommendations(engineId, albumId) {
  const delay = Math.random() * (2000 - 500) + 500; // ms
  return new Promise(resolve => {
    setTimeout(() => {
      resolve({
        engine: engineId,
...
async function loadAlbumPage(albumId, recommendationEngines, totalTimeout) {
  try {
    ...
    const result = await Promise.race([dataFetchPromise, overallTimeoutPromise]);
...
loadAlbumPage('album-123', ['engine-A', 'engine-B', 'engine-C'], 2500);
